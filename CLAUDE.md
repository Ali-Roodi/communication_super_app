...
Single SQLite database (`communication_app.db`, version 22) managed by `DatabaseHelper` singleton (`lib/core/database/`). Tables: `contacts`, `messages`, `call_logs`, `favorites`, `blocked_numbers`, `archived_threads`, `pinned_threads`, `message_categories`, `drafts`, `message_templates`, `scheduled_messages`, `thread_sim`, `speed_dial`, `message_groups`, `message_group_members`, `message_group_targets`. Constants in `AppConstants`.
