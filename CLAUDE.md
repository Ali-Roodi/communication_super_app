...
- **Starring is per *person*, not per number.** The picker used to ask «کدام شماره؟» for a contact with several; that made adding a favourite a two-step decision about a question that only matters at the moment of *calling* — and it is asked there, by the long-press sheet and by `placeCall`. The row is keyed on the contact's primary number and `_FavoriteCard` resolves the live contact from it, so the name, the photo and the numbers all stay current.
  - **…and the long-press sheet is where that question actually gets asked** (`_FavoriteCard._chooseNumber` → `pickContactNumber`), for «تماس», «ارسال پیامک» **and** the «تماس با سیم …» rows (`simCallRows(resolveNumber:)`). It did not, which made the promise above false: the sheet silently used the number the contact happened to be starred from, so somebody with a mobile and a work line could only ever be reached on the first — from the one screen built for reaching them quickly. Nothing is asked when there is no choice (one number, or a number belonging to no saved contact), and the stored number is used then, because that is the one the user starred.
...
- **A *second* call ending is not the end of the call.** Call waiting makes the ringing call `currentCall` (`onCallAdded` always does), so the ordinary case — the waiting caller gives up while the user keeps talking — reached `onStateChanged` as DISCONNECTED **for `currentCall`** and published «the call is over» with the user still on the line. `liveSuccessorTo` hands the UI to the survivor instead (an ACTIVE call beats a held or ringing leg), and only an empty phone publishes DISCONNECTED. `onCallRemoved` picks the same survivor.
- **`CallUiCoordinator` owns a LIST of call routes, not one** (`_callRoutes`). The single field was overwritten by the next push, orphaning the route it held: a call-waiting ring pushed `IncomingCallScreen` over the live call's `InCallScreen` and forgot the latter, so when everything ended the coordinator removed only what it still remembered — and the user was left staring at a dead incoming-call screen for somebody who had rung off minutes earlier, on a phone with no call on it. `_pushCall` now pushes and then removes every route it supersedes (identical to `pushReplacement` at zero transition length, but it reaches the coordinator's own routes wherever they sit in the stack), and `_dismissCallRoutes` takes all of them down. Pinned by `test/widget/call_ui_coordinator_test.dart`, which fails without it.
- **The recents thumbnail must not freeze on a call that is over** (`MainActivity.setRecentsSnapshotEnabled`, off for the whole of a call). Android captures a task's thumbnail when the activity *stops* and never refreshes it while the app stays stopped, so both ways a call ends out of sight froze the in-call screen into the switcher: `showOverLockScreen(false)`'s `moveTaskToBack` stops the activity **during** the beat Flutter deliberately lingers on the ended call, and a call ending while the user is on the home screen leaves the old thumbnail untouched. Either way the card claimed the phone was still in a call and opening it showed the ordinary app. It is switched off in `showOverLockScreen(true)` — before any `moveTaskToBack` for that call — and back on in `onResume` **only when no call is left**; turning it back on at the end of the call would let exactly the capture it prevents happen.
...
- Tap deep-links go through **one** channel: `MainActivity.consumeLaunchAction` returns a typed `{type: thread|dial|sms, …}` → `DeepLinkService.LaunchAction` → `MainNavigation._handleLaunchAction` (cold start: `consumeInitialAction`; warm: `onNewIntent`). Registered post-auth so a tap never bypasses the app lock, and the extra/`intent.data` is stripped as it is consumed so it cannot replay on the next resume.
  ...
  - **The share sheet needs its own manifest filter**, and that is why هم‌رسان was missing from the list of apps a shared piece of text could go to. `ACTION_SEND` carries its payload in `EXTRA_TEXT` and has **no data URI at all**, so the `sms:`/`smsto:` filter (which requires one) can never match it — both halves of the code had always handled the intent and nothing advertised it. A separate `<intent-filter>` of `SEND` + `DEFAULT` + `mimeType="text/plain"` is the whole fix; do not fold it back into the scheme filter.
...
- **The small icon is a white-on-transparent silhouette** (`@drawable/ic_stat_message`, and `ic_stat_call` for the three call cards), never `applicationInfo.icon`. A notification's small icon is not a picture: the system throws away every channel but alpha and tints what is left, so the *adaptive launcher* icon this used to be — a full-colour square with no transparency anywhere — was drawn as a featureless blob. There was no way to tell from the status bar which app had something to say, which is half of «نمی‌شه فهمید پیام اومده».
- **Suppressing the notification for the open conversation asks whether the user can SEE it**, not only whether the activity is resumed (`MainActivity.isResumed && visibleThreadId == threadId && isUserWatching(context)`, where watching = `PowerManager.isInteractive && !KeyguardManager.isKeyguardLocked`). On any OEM build that resumes the foreground activity *behind* the keyguard — which is exactly what `showOverLockScreen`'s gate exists for — waking a locked phone put MainActivity back in `onResume` with the lock screen still on top, and every further message from the last-opened conversation was then dropped on the floor. A user who cannot see the screen cannot have read it. `setVisibility(VISIBILITY_PRIVATE)` is likewise spelled out rather than left to the platform default.
  - Whether the lock screen draws a *card* or only the status-bar icon is the phone's own setting (One UI: «سبک اعلان بازشو» / lock-screen notification view style). Verified on the SM A336E by posting a control notification from `cmd notification post`: it behaved identically, so a missing lock-screen card is not something the app can fix from here.
...
- **«ارسال به این شماره» is for a number that belongs to nobody, and it is withheld when the typed digits ARE a listed contact's number** (`_typedNumberOwner`). It carried the digits exactly as typed, so texting a landline saved as «۰۲۱ ۸۸۱۴ ۱۸۵۹» by typing it *without* the trunk zero opened an untitled conversation on a **different thread id** than the one that person already has — the whole of "I type the contact's second number and it doesn't show their name". The contact's own row, right below it, is the answer; Google Messages does not offer the row in this case either. The test for "the same number" is `SearchText`'s own equivalence on **both** sides (`phoneForms(stored)` ∩ `PhoneQuery(query).needles`), so a *partial* number — `88141859` for that landline — deliberately does not count: those digits are a fragment of the number, not the number.
