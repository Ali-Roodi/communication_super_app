...
class _EmojiPanelState extends State<EmojiPanel> {
  ...
  Widget build(BuildContext context) {
    ...
    // The panel is given an *animating* height while the keyboard slides away
    // (see ConversationScreen), and its own contents — a fixed strip, a fixed
    // footer and the grid — cannot lay out below ~84 px without a RenderFlex
    // overflow. So it always lays out at [_kLayoutHeight] or more and is
    // clipped to whatever height it currently has, which also makes the swap
    // read as the panel being revealed rather than snapping in.
    final layoutHeight = widget.height < _kLayoutHeight
        ? _kLayoutHeight
        : widget.height;
              ...
              Expanded(
                child: LayoutBuilder(
                  builder: (context, constraints) {
                    ...
                    final columns = (constraints.maxWidth / 44).floor().clamp(
                      6,
                      10,
                    );
                    ...
                    for (final category in categories) {
                      ...
                      running +=
                          (category.emojis.length / columns).ceil() * tile;
                      slivers
                        ..add(
                          SliverToBoxAdapter(
...
/// Minimum height the panel's contents lay out at, whatever height the panel is
/// currently drawn at.
const double _kLayoutHeight = 260;
...
class _EmojiCell extends StatelessWidget {
  ...
  Future<void> _showTonePicker(BuildContext context, List<String> tones) async {
              ...
              for (final tone in tones)
                InkWell(
                  ...
                  child: Padding(
                    ...
                    child: Text(tone, style: const TextStyle(fontSize: 26)),
                  ),
                ),
            ],
          ),
...
const List<int> _kToneModifiers = [0x1F3FB, 0x1F3FC, 0x1F3FD, 0x1F3FE, 0x1F3FF];
